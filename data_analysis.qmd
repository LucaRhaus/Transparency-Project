...
```{r}
origincountry_df <- (meetings_data %>% select(Interest.Group.Name, Head.Office)) %>% group_by(Head.Office) %>% summarize(count_of_groups = n()) 
...
data <- meetings_data %>% select(Interest.Group.Name, High.Ranked, Category.Broad) %>% group_by(High.Ranked, Category.Broad) %>% summarise(Count.Of.Meetings = n())

print(data)

# Broad Categories
ggplot(data, aes(fill=Category.Broad, y=Count.Of.Meetings, x=High.Ranked)) +
   geom_bar(position="dodge", stat="identity") +
  ylim(0, 67) +
  scale_fill_paletteer_d("PrettyCols::Bold") + 
  geom_text(
    aes(label = Count.Of.Meetings),
...
ggsave(filename = "barchart meetings distribution high- vs low-ranking filled.svg", path = "./Graphics/")
```

## Most frequent interest groups

```{r}
interestgroups_frequency <- meetings_data %>% group_by(Interest.Group.Name) %>% summarise(Count.Of.Meetings = n())
interestgroups_frequency <- interestgroups_frequency[order(interestgroups_frequency$Count.Of.Meetings, decreasing = TRUE), ]  

# X-Labels umbrechen
interestgroups_frequency$Interest.Group.Name <- stringr::str_wrap(interestgroups_frequency$Interest.Group.Name, width = 50)

ggplot(interestgroups_frequency[1:10, ], aes(fill = Interest.Group.Name, x = Interest.Group.Name, y = Count.Of.Meetings)) +
  geom_bar(position = "dodge", stat = "identity") +
  ylim(0, 22) +
  geom_text(
    aes(label = Count.Of.Meetings),   
...
summarised_df <- commissioner_data %>%
  ...
  summarise(Count.Of.Meetings = n(), .groups = "drop")
...
commissioner_data_summarised_categorized <- summarised_df %>%
  complete(Commissioner.Name, Category.Detailed, fill = list(Count.Of.Meetings = 0))


ggplot(commissioner_data_summarised_categorized, aes(fill=Category.Detailed, y=Count.Of.Meetings, x=Commissioner.Name)) +
   geom_bar(position="dodge", stat="identity") +
...
timeseries_data <- timeseries_data %>%
  complete(Date.of.meeting = seq(min(Date.of.meeting), max(Date.of.meeting), by = "month"),
...
timeseries_data %>%
  ...
  ylim(0, 30) +
  annotate(geom = "text", x = as.Date("2022-02-01"), y = 0.5, color = "#ff7f0e",
           label = "Einigung EC & Parlament") +
  ...
  annotate(geom = "point", x = as.Date("2021-05-01"), y = 5, size = 7, shape = 21, fill = "transparent", color = "#ff7f0e") +
    annotate(geom = "text", x = as.Date("2023-01-15"), y = 14, color = "#ff7f0e",
           label = "Verabschiedung des    \nChips Acts durch    \ndas EU-Parlament    ") +
  annotate(geom = "point", x = as.Date("2023-04-01"), y = 1, size = 7, shape = 21, fill = "transparent", color = "#ff7f0e", radius = 0.9) +
  annotate("segment", x = as.Date("2023-04-01"), xend = as.Date("2023-01-01"), y = 1, yend = 11, colour = "#ff7f0e") +
  theme_light()
